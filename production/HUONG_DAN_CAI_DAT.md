...
- `WIX_PAGE_SETUP.md` — hướng dẫn đặt app lên Wix tại `/quan-ly-nhan-su`.
- `TEST_PLAN.md` — danh sách kiểm thử cần chạy trước khi giao cho nhân viên dùng.

Hệ thống gồm ba thành phần ghép lại: Google Sheet làm kho dữ liệu, Apps Script làm cầu nối, app HTML là giao diện đặt trên một địa chỉ web cố định (hosting bất kỳ; ở JYS dùng Wix tại `/quan-ly-nhan-su`).
...
Bạn có thể chọn một trong các cách dưới đây. JYS đã chọn nhúng vào Wix tại `/quan-ly-nhan-su` — xem file `WIX_PAGE_SETUP.md` cho chi tiết.

### Cách A — Nhúng vào Wix tại `/quan-ly-nhan-su` (khuyên dùng cho JYS)
...
2. Trong Wix Editor, tạo trang trắng có URL slug `/quan-ly-nhan-su`.
3. Bỏ trang đó khỏi menu (ẩn nó đi).
4. Thêm phần tử HTML iframe (Embed → Embed a Site) chiếm toàn trang, trỏ vào URL ở bước 1.
5. Tuỳ chọn: cấu hình page là Members-only hoặc Password protected.
